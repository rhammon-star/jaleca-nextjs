...
async function main() {
  ...
  const result = await customer.mutateResources([
    ...
    // RSA
    { entity: "Ad", operation: "create", resource: {
      ad_group: `customers/${CID}/adGroups/-3`,
      ...
      ad: {
        ...
        responsive_search_ad: {
          ...
          descriptions: [
            ...
            { text: "Frete grátis SP MG RJ ES acima de R$499. 3x sem juros. Presente perfeito pro Dia das Mães!" },
            { text: "Design moderno, bolsos funcionais e caimento perfeito para longas jornadas. Branco e colorido." },
  ...
  console.log("Recursos criados:", result.length);
  const campResult = result.find(r => r.resourceName?.includes("/campaigns/"));
  const campID = campResult?.resourceName?.split("/").pop();
  console.log(`\n✅ CAMPANHA CRIADA — ID: ${campID}`);
