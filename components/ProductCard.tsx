    ...
    <Link href={`/produto/${product.slug}`} className="group block">
      ...
      <div className="relative overflow-hidden bg-[#f5f3f0] aspect-[3/4] mb-3">
        ...
        {!isBestSeller(product.slug) && discount && (
          <span className="absolute top-3 left-3 bg-red-500 text-white text-[11px] md:text-[10px] font-bold tracking-wider md:tracking-widest uppercase px-2.5 py-1 z-10">
            -{discount}%
          </span>
        )}
        ...
        {!isBestSeller(product.slug) && hasPartialSale && (
          <span className="absolute top-3 left-3 bg-[#c4a97d] text-white text-[11px] md:text-[10px] font-bold tracking-wider md:tracking-widest uppercase px-2.5 py-1 z-10">
            PROMO
          </span>
        )}
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 z-10">
          <button
            className="w-11 h-11 md:w-8 md:h-8 rounded-full bg-white/90 backdrop-blur-sm flex items-center justify-center shadow-sm opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300 hover:bg-white hover:scale-110 active:scale-95"
          ...
          <button
            className={`w-11 h-11 md:w-8 md:h-8 bg-white/90 backdrop-blur-sm flex items-center justify-center shadow-sm opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300 hover:bg-white hover:scale-110 active:scale-95 ${inCompare ? 'text-primary' : 'text-foreground'}`}
      ...
      {/* Info */}
      <div>
        <p className="text-[12px] md:text-[10px] text-muted-foreground tracking-[0.15em] uppercase mb-0.5">Jaleca</p>
        ...
        <div className="flex flex-col gap-0.5">
          {isOnSale && product.regularPrice ? (
            <>
              <span className="text-[12px] md:text-[10px] text-muted-foreground">
