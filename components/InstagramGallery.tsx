...
export async function InstagramGallery({
  ...
  const [posts, tagged] = await Promise.all([getPosts(), getTaggedPosts()])
  const feedPosts = posts.slice(0, maxItems)
  const taggedPosts = tagged.slice(0, maxItems)
  ...
  return (
    <>
      <section className="mt-12">
        ...
        <PostGrid posts={feedPosts} label="Post Jaleca no Instagram" />
      </section>
      ...
      {taggedPosts.length > 0 && (
        <section className="mt-16">
          ...
          <PostGrid posts={taggedPosts} label="Cliente usando jaleco Jaleca" />
        </section>
      )}
    </>
  )
