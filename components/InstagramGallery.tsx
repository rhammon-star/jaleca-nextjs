...
async function getPosts(): Promise<InstagramPost[]> {
  try {
    ...
    const url = `https://graph.facebook.com/v25.0/${igId}/media?fields=id,caption,media_type,media_url,thumbnail_url,permalink,timestamp&limit=10&access_token=${token}`
...
export async function InstagramGallery(_props: { maxItems?: number; title?: string } = {}) {
  const [stories, tagged, feed] = await Promise.all([getStories(), getTaggedPosts(), getPosts()])
    ...
    <>
      {stories.length > 0 && (
        <section className="mt-12">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="https://www.instagram.com/jaleca.oficial/" target="_blank" rel="noopener noreferrer" className="text-sm text-[#6b6b6b] hover:text-[#1a1a1a] transition-colors">
              Ver no Instagram →
            </Link>
          </div>
      ...
      {tagged.length > 0 && (
        <section className="mt-16">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold">Clientes usando Jaleca</h2>
            <span className="text-sm text-[#6b6b6b]">marcaram @jaleca.oficial</span>
          </div>
