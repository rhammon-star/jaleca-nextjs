...
export const metadata: Metadata = {
  title: 'Conjunto Profissional para Farmacêutico: Elegância e Conforto | Jaleca 2026',
  ...
  alternates: { canonical: 'https://jaleca.com.br/conjunto-para-farmaceutico' },
  openGraph: {
    ...
    url: 'https://jaleca.com.br/conjunto-para-farmaceutico',
    siteName: 'Jaleca',
    locale: 'pt_BR',
    type: 'article',
  },
...
const schemaFaq = {
  ...
  '@type': 'FAQPage',
  mainEntity: [
    { '@type': 'Question', name: 'Farmacêutico pode usar qualquer cor de uniforme?', acceptedAnswer: { '@type': 'Answer', text: 'O CRF recomenda branco ou cores claras para farmácias. Em ambientes hospitalares, existem protocolos específicos de cor. Na dúvida, o branco é sempre a escolha segura.' } },
    { '@type': 'Question', name: 'Qual o tecido ideal para uniforme de farmacêutico?', acceptedAnswer: { '@type': 'Answer', text: 'O tecido com elastano é o mais indicado porque oferece flexibilidade, memória de forma e é mais fácil de lavar — essencial para quem trabalha em ambiente com exposição a produtos químicos.' } },
    { '@type': 'Question', name: 'Conjunto ou jaleco: qual é melhor para farmácia?', acceptedAnswer: { '@type': 'Answer', text: 'O jaleco é mais comum em farmácias de manipulação e hospitalar. O conjunto completo é ideal para farmácias comerciais e laboratórios onde há mais interação com clientes.' } },
    { '@type': 'Question', name: 'O uniforme de farmacêutico precisa de identificação?', acceptedAnswer: { '@type': 'Answer', text: 'Sim. O CRF exige que o farmacêutico use crachá de identificação com nome e número do registro profissional. O uniforme branco com bordado do nome transmite mais credibilidade.' } },
    { '@type': 'Question', name: 'Como manter o uniforme branco sem manchas?', acceptedAnswer: { '@type': 'Answer', text: 'Evite contato direto com produtos químicos. Lave imediatamente em caso de respingos. Use sabão neutro e evite alvejante. A Jaleca oferece tecidos que mantêm o branco por mais tempo.' } },
  ],
}
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Para Farmacêutico', item: 'https://jaleca.com.br/conjunto-para-farmaceutico' },
  ],
}
...
async function getConjuntos(): Promise<WooProduct[]> {
  try {
    ...
    const farmaceuticoSlugs = PROFESSION_PRODUCT_SLUGS['farmaceutico'] ?? []
    const products = data?.products?.nodes ?? []
    return products.filter(p => farmaceuticoSlugs.includes(p.slug) || p.slug.includes('conjunto')).slice(0, 6)
  } catch {
    return []
  }
}
...
async function getHeroImage(): Promise<{ src: string; alt: string } | null> {
  try {
    const data = await graphqlClient.request<{ product: { name: string; image: { sourceUrl: string; altText: string } } }>(
      GET_PRODUCT_BY_SLUG,
      { slug: 'conjunto-profissional-farmaceutico-jaleca' }
    )
...
async function getBlogPosts(): Promise<WPPost[]> {
  try {
    const posts = await getPosts({ per_page: 3, search: 'farmacia uniforme' })
...
export default async function ConjuntoFarmaceuticoPage() {
      ...
      <main style={{ fontWeight: 300 }}>
        ...
        >
          ...
          >
            ...
            >
              Conjunto<br />
              <em style={{ fontStyle: 'italic', fontWeight: 300 }}>para Farmacêutico</em>
            </h1>
            <p style={{ fontSize: '1rem', fontWeight: 300, color: '#6b6b6b', maxWidth: 420, marginBottom: '2.5rem', lineHeight: 1.8 }}>
              Elegância e conforto para farmácia, laboratório e escritório.
            </p>
              ...
              <div className="flex justify-between items-end flex-wrap gap-4 mb-10">
                <div>
                  ...
                  <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
                    Conjuntos para<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>Farmacêuticos</em>
                  </h2>
                </div>
            ...
            <div className="grid" style={{ gridTemplateColumns: '280px 1fr', gap: 'clamp(3rem,6vw,6rem)', alignItems: 'start' }}>
              <aside style={{ position: 'sticky', top: 80 }}>
                ...
                <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: '1.8rem', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a', marginBottom: '1.5rem' }}>
                  Como escolher o uniforme ideal para farmácia
                </h2>
                <nav>
                  <ul style={{ listStyle: 'none' }}>
                    {[
                      { label: 'Tecido e composição', anchor: '#tecido' },
                      { label: 'Cores e normas do CRF', anchor: '#cores' },
                      { label: 'Identificação profissional', anchor: '#id' },
                      { label: 'Conforto em turnos longos', anchor: '#turno' },
              ...
              <article>
                {[
                    ...
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Farmacêutico trabalha em ambiente que exige higiene constante. O tecido do uniforme precisa aguentar lavagens frequentes sem perder a aparência.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Na Jaleca, os conjuntos têm elastano que oferece flexibilidade e memória de forma, retornando ao original após cada uso.
                        </p>
                        <div style={{ background: '#1a1a1a', color: '#fff', padding: '1.5rem 2rem', margin: '2rem 0', borderLeft: '3px solid #c8c4bc' }}>
                          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.95rem', fontStyle: 'italic', fontWeight: 300, margin: 0 }}>
                            "O uniforme certo te acompanha o dia inteiro — sem apertar, sem marcar, sem perder a apresentação."
                          </p>
                        </div>
                      </>
                    ),
                  ...
                  {
                    id: 'cores',
                    title: 'Cores e normas do CRF',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O CRF recomenda cores claras, principalmente branco, para farmacêuticos em farmácias comerciais e de manipulação. Cores discretas são aceitas em laboratórios.
                        </p>
                        <ul style={{ listStyle: 'none', margin: '1.2rem 0 1.5rem' }}>
                          {[
                            'Branco — o mais tradicional e exigido em farmácias comerciais',
                            'cores claras — azul claro, verde suave, para ambientes menos formais',
                            'Preto e cinza — aceitos em farmácias hospitalares e laboratórios',
                          ].map(item => (
                  ...
                  {
                    id: 'turno',
                    title: 'Conforto em turnos longos',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Um turno na farmácia pode durar 8 horas ou mais. O elastano permite que o uniforme acompanha o corpo sem apertar ou causar desconforto.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          A calça do conjunto Jaleca tem cós com elastano que não marca a cintura mesmo após sentado por horas.
                        </p>
                      </>
                    ),
                  },
                  {
                    ...
                    body: (
                      <>
                        ...
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Para manter o branco impecable, use sabão neutro e seque à sombra. Evite alvejante que pode danificar as fibras.
                        </p>
                      </>
                    ),
                  },
        ...
        <section style={{ background: '#fff', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
              Perguntas sobre conjunto<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>para farmacêutico</em>
            </h2>
            <FaqAccordion />
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: '2px', background: '#e5e0d8', marginTop: '3rem' }}>
              {posts.length > 0 ? posts.map(post => {
                ...
                [
                  ...
                  { title: 'A importância do uniforme na imagem profissional', href: '/blog/uniforme-imagem', tag: 'Carreira', excerpt: 'Como a vestimenta correta impacta a percepção de clientes no ambiente farmacêutico.' },
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4" style={{ gap: '1px', background: 'rgba(255,255,255,0.08)' }}>
              {[
                ...
                { label: 'Professor', href: '/uniforme-para-professor', desc: 'Guia completo' },
        ...
        <section style={{ background: '#f9f7f4', padding: 'clamp(5rem,10vw,9rem) clamp(1.5rem,5vw,4rem)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2.5rem,5vw,4.5rem)', fontWeight: 400, lineHeight: 1.1, maxWidth: 700, margin: '0 auto 1rem', color: '#1a1a1a' }}>
              O conjunto certo<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>transmite credibilidade</em>
            </h2>
