...
const schemaArticle = {
  '@context': 'https://schema.org',
  '@type': 'Article',
  headline: 'Conjunto Profissional para Advogado: Elegância e Conforto',
  description: 'Guia completo do conjunto profissional para advogado: tecido premium, modelagem elegante, conforto para longas audiências e custo-benefício.',
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Para Advogado', item: 'https://jaleca.com.br/conjunto-para-advogado' },
  ],
}
...
async function getConjuntos(): Promise<WooProduct[]> {
  try {
    ...
    const advogadoSlugs = PROFESSION_PRODUCT_SLUGS['advogado'] ?? []
    const products = data?.products?.nodes ?? []
    return products.filter(p => advogadoSlugs.includes(p.slug) || p.slug.includes('conjunto')).slice(0, 6)
  } catch {
    return []
  }
}
...
async function getHeroImage(): Promise<{ src: string; alt: string } | null> {
  try {
    const data = await graphqlClient.request<{ product: { name: string; image: { sourceUrl: string; altText: string } } }>(
      GET_PRODUCT_BY_SLUG,
      { slug: 'conjunto-profissional-advogado-jaleca' }
    )
...
async function getBlogPosts(): Promise<WPPost[]> {
  try {
    const posts = await getPosts({ per_page: 3, search: 'advogado uniforme' })
...
export default async function ConjuntoAdvogadoPage() {
      ...
      <main style={{ fontWeight: 300 }}>
        ...
        >
          ...
          >
            ...
            >
              Conjunto<br />
              <em style={{ fontStyle: 'italic', fontWeight: 300 }}>para Advogado</em>
            </h1>
            <p style={{ fontSize: '1rem', fontWeight: 300, color: '#6b6b6b', maxWidth: 420, marginBottom: '2.5rem', lineHeight: 1.8 }}>
              Elegância e conforto para audiências, escritório e tribunais.
            </p>
              ...
              <div className="flex justify-between items-end flex-wrap gap-4 mb-10">
                <div>
                  ...
                  <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
                    Conjuntos para<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>Advogados</em>
                  </h2>
                </div>
            ...
            <div className="grid" style={{ gridTemplateColumns: '280px 1fr', gap: 'clamp(3rem,6vw,6rem)', alignItems: 'start' }}>
              <aside style={{ position: 'sticky', top: 80 }}>
                ...
                <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: '1.8rem', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a', marginBottom: '1.5rem' }}>
                  Como escolher o conjunto ideal para advocacia
                </h2>
                <nav>
                  <ul style={{ listStyle: 'none' }}>
                    {[
                      { label: 'Tecido e composição', anchor: '#tecido' },
                      { label: 'Modelagem e caimento', anchor: '#modelagem' },
                      { label: 'Cores e formalidade', anchor: '#cores' },
                      { label: 'Conforto em audiências longas', anchor: '#audiencia' },
              ...
              <article>
                {[
                    ...
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Advogado precisa de um uniforme que aguente o ritmo intenso do escritório sem perder a aparência profissional.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Na Jaleca, os conjuntos têm elastano no tecido. Isso permite que o roupa accompagne os movimentos sem apertar ou marcar — essencial para quem passa horas em audiências ou reuniões.
                        </p>
                        <div style={{ background: '#1a1a1a', color: '#fff', padding: '1.5rem 2rem', margin: '2rem 0', borderLeft: '3px solid #c8c4bc' }}>
                          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.95rem', fontStyle: 'italic', fontWeight: 300, margin: 0 }}>
                            "O conjunto certo não te impede de se mover. Ele te acompanha — do escritório ao tribunal."
                          </p>
                        </div>
                      </>
                    ),
                  ...
                  {
                    id: 'modelagem',
                    title: 'Modelagem e caimento: elegância sem sacrifício',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          A modelagem é o que diferencia um uniforme profissional de um terno comuns. Na Jaleca, cada conjunto é desenhado para o dia a dia de trabalho.
                        </p>
                  ...
                  {
                    id: 'cores',
                    title: 'Cores e formalidade no ambiente jurídico',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          As cores mais tradicionais na advocacia são preto, grafite e azul marinho. Elas transmitem seriedade e são aceitas em qualquer tribunal.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Tons mais claros como cinza claro podem funcionar em escritórios modernos e tribunais menos formais. A Jaleca oferece 12 cores disponíveis.
                        </p>
                      </>
                    ),
                  },
                  {
                    id: 'audiencia',
                    title: 'Conforto em audiências longas',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Uma audiência pode durar horas. O tecido com elastano faz a diferença porque não aperta os ombros nem marca as costas quando você se senta por muito tempo.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          A calça do conjunto Jaleca tem cós com elastano, o que permite movimento total sem apertar a cintura durante audiências longas.
                        </p>
                      </>
                    ),
                  },
                  {
                    ...
                    body: (
                      <>
                        ...
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Para manter a aparência profissional, evite alvejante e seque à sombra. O ferro de passar deve estar em temperatura média.
                        </p>
                      </>
                    ),
                  },
        ...
        <section style={{ background: '#fff', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
              Perguntas sobre conjunto<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>para advogado</em>
            </h2>
            <FaqAccordion />
          </div>
        </section>
          ...
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: '2px', background: '#e5e0d8', marginTop: '3rem' }}>
              {posts.length > 0 ? posts.map(post => {
                ...
                [
                  { title: 'Como lavar e conservar seu uniforme profissional', href: '/blog/como-lavar-uniformes', tag: 'Cuidados', excerpt: 'Erros simples de lavagem aceleram o desgaste e encurtam a vida útil do uniforme. Veja o guia completo.' },
                  { title: 'A importância do uniforme na imagem profissional', href: '/blog/uniforme-imagem', tag: 'Carreira', excerpt: 'Como a vestimenta correta impacta a percepção de clientes e colegas no ambiente jurídico.' },
                  { title: 'Como escolher o tamanho certo do uniforme', href: '/medidas', tag: 'Guia de Tamanhos', excerpt: 'Passo a passo para medir busto, cintura e quadril e encontrar o tamanho ideal na grade Jaleca.' },
                ].map(post => (
