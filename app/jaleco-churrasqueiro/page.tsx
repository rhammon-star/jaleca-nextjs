...
export default async function JalecoChurrasqueiroPage() {
  ...
  return (
    <>
      ...
      {(() => { const arr = buildProductListSchema(produtos, 'https://jaleca.com.br/jaleco-churrasqueiro'); return arr ? arr.map((s, i) => <script key={'p'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
      {(() => { const arr = buildReviewSchema(placeData?.reviews, 'https://jaleca.com.br/jaleco-churrasqueiro', "Jaleco para churrasqueiro", produtos); return arr ? arr.map((s, i) => <script key={'r'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
