...
const schemaArticle = {
  '@context': 'https://schema.org',
  '@type': 'Article',
  headline: 'Jaleco para Advogado: Elegância e Autoridade no Direito',
  description: 'Guia completo do jaleco para advogado: modelo ideal, tecido premium, caimento impecável, normas da OAB e custo-benefício.',
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Jaleco para Advogado', item: 'https://jaleca.com.br/jaleco-advogado' },
  ],
}
...
async function getJalecos(): Promise<WooProduct[]> {
  try {
    ...
    // Filtra por profissão advogado
    const slugs = PROFESSION_PRODUCT_SLUGS['advogado'] ?? []
...
export default async function JalecoAdvogadoPage() {
  const [produtos, posts, placeData, heroImg] = await Promise.all([
    getJalecos(),
    getCachedBlogPosts('jaleco'),
    getGooglePlaceData(),
    getCachedHeroImage(getHeroImageSlug('advogado') ?? ''),
  ])
  ...
  return (
    <>
      ...
      {(() => { const arr = buildProductListSchema(produtos, 'https://jaleca.com.br/jaleco-advogado'); return arr ? arr.map((s, i) => <script key={'p'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
      {(() => { const arr = buildReviewSchema(placeData?.reviews, 'https://jaleca.com.br/jaleco-advogado', "Jaleco para advogado", produtos); return arr ? arr.map((s, i) => <script key={'r'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
      ...
      <main style={{ fontWeight: 300 }}>
        ...
        <HeroCommercial
          ...
          h1Line1="Jaleco para"
          h1Line2="Advogado"
          description="Elegância e autoridade no direito. Tecido premium, caimento impecável."
          startingPrice="R$220"
          ...
          <section id="produtos" className="px-4 py-12 lg:px-16 lg:py-20" style={{ background: '#f9f7f4', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
            <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
              <div className="mb-10">
                <div>
                  ...
                  <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
                    Jalecos para<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>Advogados</em>
                  </h2>
                </div>
                </div>
              ...
              <div className="flex justify-center mt-8">
                <Link href={getVerMaisUrl('advogado')} style={{ display: 'inline-flex', padding: '0.9rem 2rem', fontSize: '0.78rem', fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', border: '1px solid #1a1a1a', color: '#1a1a1a' }}>
                  Ver mais →
                </Link>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8 lg:gap-24" style={{ gap: 'clamp(3rem,6vw,6rem)', alignItems: 'start' }}>
              <aside style={{ position: 'sticky', top: 80 }}>
                ...
                <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: '1.8rem', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a', marginBottom: '1.5rem' }}>
                  Como escolher o jaleco ideal para advocacia
                </h2>
                <nav className="hidden lg:block">
                  <ul style={{ listStyle: 'none' }}>
                    {[
                      { label: 'Jaleco e traje forense', anchor: '#traje' },
                      { label: 'Modelo Profissional vs Slim', anchor: '#modelagem' },
                      { label: 'Cores adequadas', anchor: '#cores' },
                      { label: 'Bordado com OAB', anchor: '#bordado' },
                      { label: 'Onde usar o jaleco', anchor: '#uso' },
                    ].map(item => (
              ...
              <article>
                {[
                  {
                    id: 'traje',
                    title: 'Jaleco e traje forense: quando usar cada um',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O jaleco não substitui a toga nas sessões solenes do tribunal. Mas é peça fundamental em audiências de custódia, defensorias públicas e atendimentos de jurídica popular.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O advogado que atua em contato direto com clientes em situação vulnerável — como na área criminal ou de família — usa o jaleco como sinal de profissionalismo e autoridade moral.
                        </p>
                        <div style={{ background: '#1a1a1a', color: '#fff', padding: '1.5rem 2rem', margin: '2rem 0', borderLeft: '3px solid #c8c4bc' }}>
                          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.95rem', fontStyle: 'italic', fontWeight: 300, margin: 0 }}>
                            "O jaleco comunica ao cliente que ele está diante de um profissional sério. É uma extensão da credibilidade do advogado."
                          </p>
                        </div>
                      </>
                    ),
                  ...
                  {
                    id: 'modelagem',
                    title: 'Modelo Profissional ou Slim: qual escolher',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Para o meio jurídico, o modelo Profissional transmite mais autoridade — o corte estruturado passa solidez e tradição. É a escolha clássica para tribunais e escritórios tradicionais.
                        </p>
                        <ul style={{ listStyle: 'none', margin: '1.2rem 0 1.5rem' }}>
                          {[
                            'Profissional — Corte amplo e estruturado, transmite formalidade máxima, ideal para audiências e sessões no tribunal',
                            'Slim — Corte ajustado, visual moderno, aceito em escritórios inovadores e departamentos jurídicos de empresas',
                          ].map(item => (
                  ...
                  },
                  {
                    ...
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O branco é o mais tradicional e seguro. Mas a cor escura — preto ou marinho — transmite ainda mais seriedade e é cada vez mais aceita em escritórios de advocacia modernos.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          A OAB não restringe a cor do jaleco em situações onde ele é usado. Para atuacao em defensorias e audiências de custódia, o preto passa autoridade e respeito.
                        </p>
                      </>
                    ),
                  },
                  ...
                  {
                    id: 'uso',
                    title: 'Onde usar o jaleco na advocacia',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O jaleco é apropriado em atendimentos de defensoria pública, audiências de custódia, juizados especiais e qualquer situação em que o advogado preste assistência jurídica direta.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O uso do jaleco fora do ambiente de trabalho deve ser evitado — a peça deve ser reservada exclusivamente para a atuação profissional, mantendo a higiene e a imagem.
                        </p>
                      </>
                    ),
                  },
        ...
        <section style={{ background: '#fff', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
          <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
              Perguntas sobre jaleco<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>para advogado</em>
            </h2>
            <FaqAccordion />
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: '2px', background: '#e5e0d8', marginTop: '3rem' }}>
              {posts.length > 0 ? posts.map(post => {
                ...
                [
                  ...
                  { title: 'Jaleco preto: elegância no ambiente jurídico', href: '/blog', tag: 'Advocacia', excerpt: 'Por que cada vez mais advogados estão escolhendo o jaleco preto e o que isso comunica ao cliente.' },
        ...
        {/* ── TOPICAL AUTHORITY — Outros profissionais ── */}
