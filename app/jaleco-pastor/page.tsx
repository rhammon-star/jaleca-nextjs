...
export const metadata: Metadata = {
  title: { absolute: 'Jaleco Pastor com Postura Profissional — Jaleca' },
  description: 'Nota 4,9 no Google. Jaleco para pastores com caimento alfaiataria, elastano e modelagem slim — PP ao G3. Frete grátis Sudeste acima de R$499.',
  alternates: { canonical: 'https://jaleca.com.br/jaleco-pastor' },
  openGraph: {
    title: 'Jaleco Pastor — Caimento Alfaiataria, Postura Profissional — Jaleca',
    description: 'Nota 4,9 no Google. Jaleco para pastores com elastano, modelagem slim, PP ao G3. Frete grátis Sudeste acima de R$499.',
    url: 'https://jaleca.com.br/jaleco-pastor',
    siteName: 'Jaleca',
    locale: 'pt_BR',
    type: 'article',
  },
  twitter: {
    ...
    title: 'Jaleco Pastor — Postura Profissional — Jaleca',
    description: 'Jaleco premium para pastores. Caimento alfaiataria, elastano, do PP ao G3.',
...
const schemaArticle = {
  '@context': 'https://schema.org',
  '@type': 'Article',
  headline: 'Jaleco para Pastor: Tecido Premium, Caimento Perfeito',
  description: 'Guia completo do jaleco para pastor: tecido premium, caimento perfeito, modelo Slim vs Profissional e custo-benefício.',
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Jaleco para Pastor', item: 'https://jaleca.com.br/jaleco-pastor' },
  ],
}
...
async function getJalecos(): Promise<WooProduct[]> {
  try {
    ...
    // Filtra por profissão pastor
    const slugs = PROFESSION_PRODUCT_SLUGS['pastor'] ?? []
...
export default async function JalecoDentistaPage() {
  const [produtos, posts, placeData, heroImg] = await Promise.all([
    ...
    getGooglePlaceData(),
    getCachedHeroImage(getHeroImageSlug('pastor') ?? ''),
  ])
  ...
  return (
    <>
      ...
      {(() => { const arr = buildProductListSchema(produtos, 'https://jaleca.com.br/jaleco-pastor'); return arr ? arr.map((s, i) => <script key={'p'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
      {(() => { const arr = buildReviewSchema(placeData?.reviews, 'https://jaleca.com.br/jaleco-pastor', "Jaleco para pastor", produtos); return arr ? arr.map((s, i) => <script key={'r'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
      ...
      <main style={{ fontWeight: 300 }}>
        ...
        {produtos.length > 0 && (
          <section id="produtos" className="px-4 py-12 lg:px-16 lg:py-20" style={{ background: '#f9f7f4', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
            <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
              <div className="mb-10">
                <div>
                  <div style={{ fontSize: '0.7rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: '#c8c4bc', marginBottom: '0.75rem' }}>Coleção religioso</div>
                  <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
                    Jalecos para<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>Pastors</em>
                  </h2>
                </div>
                </div>
              ...
              <div className="flex justify-center mt-8">
                <Link href={getVerMaisUrl('pastor')} style={{ display: 'inline-flex', padding: '0.9rem 2rem', fontSize: '0.78rem', fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', border: '1px solid #1a1a1a', color: '#1a1a1a' }}>
                  Ver mais →
                </Link>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8 lg:gap-24" style={{ gap: 'clamp(3rem,6vw,6rem)', alignItems: 'start' }}>
              <aside style={{ position: 'sticky', top: 80 }}>
                ...
                <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: '1.8rem', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a', marginBottom: '1.5rem' }}>
                  Como escolher o jaleco ideal para liturgia
                </h2>
                    ...
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Jaleco de pastor aguenta muita lavagem e mancha. Precisa ser confortável por horas.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
        ...
        <section style={{ background: '#fff', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
          <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
              Perguntas sobre jaleco<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>para pastor</em>
            </h2>
            <FaqAccordion />
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: '2px', background: '#e5e0d8', marginTop: '3rem' }}>
              {posts.length > 0 ? posts.map(post => {
                ...
                [
                  ...
                  { title: 'Jaleco branco: tradição e protocolos em liturgia', href: '/blog', tag: 'Liturgia', excerpt: 'Por que o branco é tão usado em liturgia e o que recomendam sobre cores e vestimenta.' },
