...
export const metadata: Metadata = {
  ...
  alternates: { canonical: 'https://jaleca.com.br/uniforme-para-professor' },
  openGraph: {
    title: 'Uniforme para Professor | Elegância e Conforto — Jaleca',
    description: 'Uniforme profissional completo para professores. Tecido premium, caimento impecável, preço justo. Do PP ao G3.',
    url: 'https://jaleca.com.br/uniforme-para-professor',
    siteName: 'Jaleca',
    locale: 'pt_BR',
    type: 'article',
  },
...
const schemaFaq = {
  ...
  '@type': 'FAQPage',
  mainEntity: [
    { '@type': 'Question', name: 'Professor pode usar qualquer tipo de uniforme?', acceptedAnswer: { '@type': 'Answer', text: 'Na maioria das escolas e universidades, há um dress code definido. College wear (blazer, calça social, camisa) é o mais comum. Em escolas maiscasuais, camisa e calça chinos são aceitos. Verifique sempre o manual da instituição.' } },
    { '@type': 'Question', name: 'Qual o tecido ideal para uniforme de professor?', acceptedAnswer: { '@type': 'Answer', text: 'O tecido com elastano é o mais indicado por oferecer flexibilidade e memória de forma, mantendo a elegância mesmo após longas horas de aula. Tecidos que não amassam facilitam a rotina.' } },
    { '@type': 'Question', name: 'Uniforme de professor precisa ser muito formal?', acceptedAnswer: { '@type': 'Answer', text: 'Depende da instituição. Escolas tradicionais pedem mais formalidade (blazer, gravata). Escolas modernas aceitam casual mais elegante. O importante é transmitir profissionalismo e credibilidade.' } },
    { '@type': 'Question', name: 'O elastano desbota rápido no uniforme?', acceptedAnswer: { '@type': 'Answer', text: 'Não. Os tecidos Jaleca com elastano são projetados para manter a cor e a forma mesmo após múltiplas lavagens, desde que seguidas as instruções de conservação.' } },
    { '@type': 'Question', name: 'Como manter o uniforme de professor impecável?', acceptedAnswer: { '@type': 'Answer', text: 'Lave em água fria ou morna, evite alvejante, e passe em temperatura média. Tecidos com elastano mantêm a forma e reduzem a necessidade de passar. Guarde em cabide para evitar rugas.' } },
  ],
}
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 2, name: 'Uniformes', item: 'https://jaleca.com.br/produtos?categoria=uniformes' },
    { '@type': 'ListItem', position: 3, name: 'Para Professor', item: 'https://jaleca.com.br/uniforme-para-professor' },
  ],
}

async function getUniformes(): Promise<WooProduct[]> {
  try {
    const data = await graphqlClient.request<{ products: { nodes: WooProduct[] } }>(GET_PRODUCTS, {
      first: 50,
    })
    const professorSlugs = PROFESSION_PRODUCT_SLUGS['professor'] ?? []
    const products = data?.products?.nodes ?? []
    return products.filter(p => professorSlugs.includes(p.slug) || p.slug.includes('conjunto') || p.slug.includes('uniforme')).slice(0, 6)
  } catch {
    return []
  }
}
      ...
      <main style={{ fontWeight: 300 }}>
        ...
        <div style={{ background: '#f9f7f4', borderBottom: '1px solid #e5e0d8', padding: '0.85rem clamp(1.5rem,5vw,4rem)' }}>
          <ol className="flex items-center gap-2 max-w-[1200px] mx-auto" style={{ listStyle: 'none' }}>
            {[
              { label: 'Início', href: '/' },
              { label: 'Uniformes', href: '/produtos?categoria=uniformes' },
              { label: 'Para Professor', href: null },
            ].map((crumb, i, arr) => (
        ...
        >
          ...
          >
            ...
            >
              Uniforme para<br />
              <em style={{ fontStyle: 'italic', fontWeight: 300 }}>Professor</em>
            </h1>
            <p style={{ fontSize: '1rem', fontWeight: 300, color: '#6b6b6b', maxWidth: 420, marginBottom: '2.5rem', lineHeight: 1.8 }}>
              Elegância e conforto para a sala de aula e vida acadêmica.
            </p>
            <div className="flex gap-4 flex-wrap">
              <Link href="/produtos?categoria=uniformes" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.9rem 2rem', background: '#1a1a1a', color: '#fff', fontSize: '0.78rem', fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', border: '1px solid #1a1a1a' }}>
                Ver Uniformes →
              </Link>
              <Link href="/produtos?categoria=conjuntos" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.9rem 2rem', background: 'transparent', color: '#1a1a1a', fontSize: '0.78rem', fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', border: '1px solid #1a1a1a' }}>
                Ver Conjuntos →
              </Link>
            </div>
          ...
          <section style={{ background: '#f9f7f4', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
            <div style={{ maxWidth: 1200, margin: '0 auto' }}>
              <div className="flex justify-between items-end flex-wrap gap-4 mb-10">
                <div>
                  <div style={{ fontSize: '0.7rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: '#c8c4bc', marginBottom: '0.75rem' }}>Coleção educação</div>
                  <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
                    Uniformes para<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>Professores</em>
                  </h2>
                </div>
                <Link href="/produtos?categoria=uniformes" style={{ display: 'inline-flex', padding: '0.9rem 2rem', fontSize: '0.78rem', fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', border: '1px solid #1a1a1a', color: '#1a1a1a' }}>
                  Ver todos →
                </Link>
              </div>
            ...
            <div className="grid" style={{ gridTemplateColumns: '280px 1fr', gap: 'clamp(3rem,6vw,6rem)', alignItems: 'start' }}>
              <aside style={{ position: 'sticky', top: 80 }}>
                ...
                <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: '1.8rem', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a', marginBottom: '1.5rem' }}>
                  Como escolher o uniforme ideal para educação
                </h2>
                <nav>
                  <ul style={{ listStyle: 'none' }}>
                    {[
                      { label: 'Tecido e composição', anchor: '#tecido' },
                      { label: 'Níveis de formalidade', anchor: '#formalidade' },
                      { label: 'Conforto em aulas longas', anchor: '#aulas' },
                      { label: 'Cores e identidade visual', anchor: '#cores' },
              ...
              <article>
                {[
                    ...
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Professor passa horas em pé, sentado, se movendo pela sala. O tecido do uniforme precisa acompanhar todo esse movimento sem perder a apresentação.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Na Jaleca, os uniformes têm elastano que oferece flexibilidade e memória de forma, mantendo a elegância mesmo após longas horas de aula.
                        </p>
                        <div style={{ background: '#1a1a1a', color: '#fff', padding: '1.5rem 2rem', margin: '2rem 0', borderLeft: '3px solid #c8c4bc' }}>
                          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.95rem', fontStyle: 'italic', fontWeight: 300, margin: 0 }}>
                            "O uniforme certo não te limita — ele te acompanha do primeiro ao último minuto de aula."
                          </p>
                        </div>
                      </>
                    ),
                  ...
                  {
                    id: 'formalidade',
                    title: 'Níveis de formalidade: de college wear a casual elegante',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O nível de formalidade do uniforme de professor depende da instituição. Colleges e universidades tradicionais pedem mais formalidade. Escolas modernas aceitam casual mais elegante.
                        </p>
                        <ul style={{ listStyle: 'none', margin: '1.2rem 0 1.5rem' }}>
                          {[
                            'College wear — blazer, camisa, calça social. Mais tradicional e formal',
                            'Smart casual — camisa, calça chino, blazer opcional. Equilibrio entre conforto e profissionalismo',
                            'Casual elegante — camisa, calça social em cores mais modernas. Para instituições menos formais',
                          ].map(item => (
                  ...
                  {
                    id: 'aulas',
                    title: 'Conforto em aulas longas',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Uma aula pode durar 50 minutos ou mais. O elastano permite que o uniforme accompagne o corpo sem apertar ou causar desconforto durante as aulas.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          A calça do uniforme Jaleca tem cós com elastano que não marca a cintura mesmo após sentado por horas em sala de aula ou em frente ao computador.
                        </p>
                      </>
                    ),
                  },
                  {
                    ...
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          As cores mais tradicionais para uniformes de professor são preto, cinza, azul marinho e branco. Elas transmitem profissionalismo e são aceitas na maioria das instituições.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Para instituições com identidade visual específica, a Jaleca oferece 12 cores disponíveis para criar uniformes personalizados que refletem a marca da escola.
                        </p>
                      </>
                    ),
                  },
                  {
                    ...
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Os uniformes Jaleca suportam lavagem doméstica até 40°C. O elastano mantém a cor e a forma mesmo após múltiplas lavagens.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Para manter a aparência profissional, lave em água fria, evite alvejante, e passe em temperatura média. Tecidos com elastano reduzem a necessidade de passar.
                        </p>
                      </>
                    ),
                  },
        ...
        <section style={{ background: '#fff', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
              Perguntas sobre uniforme<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>para professor</em>
            </h2>
            <FaqAccordion />
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: '2px', background: '#e5e0d8', marginTop: '3rem' }}>
              {posts.length > 0 ? posts.map(post => {
                ...
                [
                  ...
                  { title: 'A importância do uniforme na imagem profissional', href: '/blog/uniforme-imagem', tag: 'Carreira', excerpt: 'Como a vestimenta correta impacta a percepção de alunos e colegas.' },
        ...
        <section style={{ background: '#1a1a1a', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(1.8rem,3vw,2.5rem)', fontWeight: 400, lineHeight: 1.15, color: '#fff', marginBottom: '2.5rem' }}>
              Uniformes para outras<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>profissões</em>
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4" style={{ gap: '1px', background: 'rgba(255,255,255,0.08)' }}>
              {[
                { label: 'Advogado', href: '/conjunto-para-advogado', desc: 'Guia completo' },
                { label: 'Psicóloga', href: '/conjunto-para-psicologa', desc: 'Guia completo' },
                { label: 'Farmacêutico', href: '/conjunto-para-farmaceutico', desc: 'Guia completo' },
                { label: 'Pastor', href: '/conjunto-para-pastor', desc: 'Guia completo' },
                { label: 'Ver todos', href: '/produtos?categoria=uniformes', desc: 'Loja completa' },
              ].map(item => (
        ...
        <section style={{ background: '#f9f7f4', padding: 'clamp(5rem,10vw,9rem) clamp(1.5rem,5vw,4rem)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2.5rem,5vw,4.5rem)', fontWeight: 400, lineHeight: 1.1, maxWidth: 700, margin: '0 auto 1rem', color: '#1a1a1a' }}>
              O uniforme certo<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>eleva a credibilidade</em>
            </h2>
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              <Link href="/produtos?categoria=uniformes" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.9rem 2.5rem', background: '#1a1a1a', color: '#fff', fontSize: '0.78rem', fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', border: '1px solid #1a1a1a' }}>
                Ver Coleção Completa
              </Link>
              <Link href="/produtos?categoria=conjuntos" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.9rem 2.5rem', background: 'transparent', color: '#1a1a1a', fontSize: '0.78rem', fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', border: '1px solid #1a1a1a' }}>
                Ver Conjuntos
              </Link>
            </div>
          </div>
