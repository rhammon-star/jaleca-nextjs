...
export const metadata: Metadata = {
  title: { absolute: 'Jaleco de Psicóloga Slim — Feminino e Masculino na Jaleca' },
  description: 'Nota 4,9 no Google. Jaleco com elastano para psicólogas — branco, preto e colorido, PP ao G3. Frete grátis Sudeste acima de R$499. Troca em 7 dias.',
  alternates: { canonical: 'https://jaleca.com.br/jaleco-psicologa' },
  openGraph: {
    title: 'Jaleco para Psicóloga — Slim, Branco e Colorido — Jaleca',
    description: 'Nota 4,9 no Google. Jaleco para psicólogas com elastano, modelagem slim, PP ao G3. Frete grátis Sudeste acima de R$499.',
    url: 'https://jaleca.com.br/jaleco-psicologa',
    siteName: 'Jaleca',
    locale: 'pt_BR',
    type: 'article',
  },
  twitter: {
    ...
    description: 'Jaleco premium para psicólogas. Elastano, caimento perfeito, do PP ao G3. Frete grátis Sudeste.',
...
const schemaArticle = {
  '@context': 'https://schema.org',
  '@type': 'Article',
  headline: 'Jaleco para Psicóloga: Tecido Premium, Caimento Perfeito',
  description: 'Guia completo do jaleco para psicóloga: tecido premium, caimento perfeito, modelo Slim vs Profissional e custo-benefício.',
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Jaleco para Psicóloga', item: 'https://jaleca.com.br/jaleco-psicologa' },
  ],
}
...
async function getJalecos(): Promise<WooProduct[]> {
  try {
    ...
    // Filtra por profissão psicologa
    const slugs = PROFESSION_PRODUCT_SLUGS['psicologa'] ?? []
...
export default async function JalecoDentistaPage() {
  const [produtos, posts, placeData, heroImg] = await Promise.all([
    ...
    getGooglePlaceData(),
    getCachedHeroImage(getHeroImageSlug('psicologa') ?? ''),
  ])
  ...
  return (
    <>
      ...
      {(() => { const arr = buildProductListSchema(produtos, 'https://jaleca.com.br/jaleco-psicologa'); return arr ? arr.map((s, i) => <script key={'p'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
      {(() => { const arr = buildReviewSchema(placeData?.reviews, 'https://jaleca.com.br/jaleco-psicologa', "Jaleco para psicologa", produtos); return arr ? arr.map((s, i) => <script key={'r'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
      ...
      <main style={{ fontWeight: 300 }}>
          ...
          <section id="produtos" className="px-4 py-12 lg:px-16 lg:py-20" style={{ background: '#f9f7f4', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
            <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
              <div className="mb-10">
                <div>
                  ...
                  <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
                    Jalecos para<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>Psicólogas</em>
                  </h2>
                </div>
                </div>
              ...
              <div className="flex justify-center mt-8">
                <Link href={getVerMaisUrl('psicologa')} style={{ display: 'inline-flex', padding: '0.9rem 2rem', fontSize: '0.78rem', fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', border: '1px solid #1a1a1a', color: '#1a1a1a' }}>
                  Ver mais →
                </Link>
              </div>
            </div>
        ...
        <section style={{ background: '#fff', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
          <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
              Perguntas sobre jaleco<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>para psicóloga</em>
            </h2>
            <FaqAccordion />
          </div>
        </section>
