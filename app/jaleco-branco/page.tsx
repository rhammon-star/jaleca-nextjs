...
const schemaFaq = {
  ...
  '@type': 'FAQPage',
  mainEntity: [
    { '@type': 'Question', name: 'Como evitar manchas no jaleco branco?', acceptedAnswer: { '@type': 'Answer', text: 'Aplique sabão neutro imediatamente quando sujar. Nunca use alvejante com cloro diretamente — prefira alvejante sem cloro ou água oxigenada 10 volumes. Seque à sombra para evitar amarelamento.' } },
    { '@type': 'Question', name: 'Qual o melhor tecido para jaleco branco profissional?', acceptedAnswer: { '@type': 'Answer', text: 'Gabardine com elastano é o melhor — resistente, não amassa fácil e não desbota. Jalecos Jaleca têm 67% poliéster, 33% algodão e elastano 3-8%.' } },
    { '@type': 'Question', name: 'Jaleco branco feminino e masculino têm corte diferente?', acceptedAnswer: { '@type': 'Answer', text: 'Sim. O feminino tem busto estruturado, cintura levemente marcada e quadril com espaço. O masculino tem ombros mais largos e corte reto.' } },
    { '@type': 'Question', name: 'Jaleco com elastano desbota mais rápido?', acceptedAnswer: { '@type': 'Answer', text: 'Não. O elastano em si não causa desbotamento — o problema é excesso de temperatura na lavagem. Jalecos Jaleca mantêm cor e forma seguindo as instruções de lavagem.' } },
    { '@type': 'Question', name: 'Jaleco branco amarela com o tempo?', acceptedAnswer: { '@type': 'Answer', text: 'Pode amarelar se guardado sujo, exposto ao sol diretamente ou lavado com água muito quente. Use branqueadores ópticos e Deixe de molho em água fria com vinagre branco se começar a amarelar.' } },
  ],
}
...
const schemaArticle = {
  '@context': 'https://schema.org',
  '@type': 'Article',
  headline: 'Jaleco Branco Profissional — Conforto e Elegância para a Rotina Clínica',
  description: 'Guia completo do jaleco branco profissional: melhor tecido, como evitar manchas, diferença entre modelos feminino e masculino, e cuidados com o elastano.',
...
async function getJalecosBrancos(): Promise<WooProduct[]> {
  try {
    const data = await graphqlClient.request<{ products: { nodes: WooProduct[] } }>(GET_PRODUCTS, {
      first: 50,
    })
    const products = data?.products?.nodes ?? []
    // Filter for white jalecos - check slug and name for "branco" or "white"
    return products.filter(p =>
      (p.name?.toLowerCase().includes('branco') || p.slug?.includes('branco')) &&
      (p.name?.toLowerCase().includes('jaleco') || p.slug?.includes('jaleco'))
    ).slice(0, 6)
  } catch {
    return []
  }
...
async function getHeroImage(): Promise<{ src: string; alt: string } | null> {
  try {
    ...
    const whiteProduct = data?.products?.nodes?.find(p =>
      p.name?.toLowerCase().includes('branco') && p.name?.toLowerCase().includes('jaleco')
    )
    if (whiteProduct?.image?.sourceUrl) {
      return { src: whiteProduct.image.sourceUrl, alt: whiteProduct.image.altText || whiteProduct.name }
    }
    return null
  } catch {
    return null
...
async function getBlogPosts(): Promise<WPPost[]> {
  try {
    const posts = await getPosts({ per_page: 3, search: 'jaleco branco' })
...
export default async function JalecoBrancoPage() {
      ...
      <main style={{ fontWeight: 300 }}>
        ...
        >
          ...
          >
            ...
            >
              Jaleco<br />
              <em style={{ fontStyle: 'italic', fontWeight: 300 }}>Branco</em>
            </h1>
            <p style={{ fontSize: '1rem', fontWeight: 300, color: '#6b6b6b', maxWidth: 420, marginBottom: '2.5rem', lineHeight: 1.8 }}>
              Conforto e elegância para a rotina clínica. Tecido premium com elastano, caimento perfeito.
            </p>
          ...
          <div className="relative order-1 lg:order-2" style={{ background: '#e5e0d8', minHeight: 480, overflow: 'hidden' }}>
            {heroImg ? (
              ...
              <div style={{ width: '100%', height: '100%', background: 'linear-gradient(160deg, #f5f5f0 0%, #e8e8e0 100%)', position: 'absolute', inset: 0 }} />
            )}
          </div>
        </section>
          ...
          <section className="px-4 py-12 lg:px-16 lg:py-20" style={{ background: '#f9f7f4', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
            <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
              <div className="flex justify-between items-end flex-wrap gap-4 mb-10">
                <div>
                  <div style={{ fontSize: '0.7rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: '#c8c4bc', marginBottom: '0.75rem' }}>Coleção branco</div>
                  <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
                    Jalecos<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>Brancos</em>
                  </h2>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8 lg:gap-24" style={{ gap: 'clamp(3rem,6vw,6rem)', alignItems: 'start' }}>
              <aside style={{ position: 'sticky', top: 80 }}>
                ...
                <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: '1.8rem', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a', marginBottom: '1.5rem' }}>
                  Como cuidar do seu jaleco branco
                </h2>
                <nav className="hidden lg:block">
                  <ul style={{ listStyle: 'none' }}>
                    {[
                      { label: 'Tecido e composição', anchor: '#tecido' },
                      { label: 'Como evitar manchas', anchor: '#manchas' },
                      { label: 'Lavagem e conservação', anchor: '#lavagem' },
                      { label: 'Masculino vs feminino', anchor: '#corte' },
                      { label: 'Elastano e durabilidade', anchor: '#elastano' },
                    ].map(item => (
              ...
              <article>
                {[
                    ...
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Jaleco branco profissional precisa suportar muita lavagem sem amarelar ou perder a forma. Por isso a composição do tecido faz toda a diferença.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Na Jaleca, todos os jalecos brancos têm gabardine com elastano — 67% poliéster, 33% algodão e 3-8% elastano. Isso garante resistência, conforto e memória do tecido.
                        </p>
                        <div style={{ background: '#1a1a1a', color: '#fff', padding: '1.5rem 2rem', margin: '2rem 0', borderLeft: '3px solid #c8c4bc' }}>
                          <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.95rem', fontStyle: 'italic', fontWeight: 300, margin: 0 }}>
                            "O jaleco branco ideal? Aquele que você nem sente que está vestindo, mas que continua impecável mesmo depois de muitos lavagem."
                          </p>
                        </div>
                      </>
                    ),
                  ...
                  {
                    id: 'manchas',
                    title: 'Como evitar manchas no jaleco branco',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Manchas em jaleco branco são o maior desafio. A regra de ouro: nunca deixe secar sujo. Quanto mais rápido você lavar, maiores as chances de sair completa.
                        </p>
                        <ul style={{ listStyle: 'none', margin: '1.2rem 0 1.5rem' }}>
                          {[
                            'Sangue: água fria + sabão neutro imediatamente. Nunca use água quente.',
                            'Tinta de caneta: aplique álcool 70% diretamente na mancha, depois lave.',
                            'Café: água fria primeiro, depois sabão neutro. Evite água quente.',
                            'Graxa: talco ou amido de milho na mancha por 15 min antes de lavar.',
                          ].map(item => (
                  ...
                  {
                    id: 'lavagem',
                    title: 'Lavagem e conservação do jaleco branco',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Jalecos Jaleca suportam lavagem a 60°C. Mas para manter a brancura por mais tempo, recomendamos 40°C com sabão neutro. Nunca use alvejante com cloro — prefira alvejante sem cloro ou água oxigenada 10 volumes.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Se o jaleco começar a amarelar, deixe de molho em água fria com 2 colheres de vinagre branco por 30 minutos, depois lave normalmente. Seque à sombra para evitar exposição solar direta.
                        </p>
                      </>
                    ),
                  },
                  {
                    id: 'corte',
                    title: 'Jaleco branco masculino vs feminino',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O corte faz diferença no caimento e no conforto durante o turno. Jalecos femininos têm estrutura no busto, cintura levemente marcada e espaço no quadril. Jalecos masculinos têm ombros mais largos e corte reto.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          Todos os modelos Jaleca têm elastano, independente do corte. Isso garante que o jaleco acompanha o movimento sem apertar ou ficar largo com o tempo.
                        </p>
                      </>
                    ),
                  },
                  {
                    id: 'elastano',
                    title: 'Elastano e durabilidade do jaleco branco',
                    body: (
                      <>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O elastano (3-8%) adiciona memória ao tecido — ele volta ao formato original após cada movimento. Isso é essencial para quem trabalha horas em pé ou com os braços elevados.
                        </p>
                        <p style={{ fontSize: '0.97rem', color: '#444', lineHeight: 1.85, marginBottom: '1.2rem', fontWeight: 300 }}>
                          O elastano não desbota mais rápido se você seguir as instruções de lavagem. O problema real é temperatura excessiva (água quente ou secadora). Seguindo os cuidados, seu jaleco branco dura anos.
                        </p>
                      </>
                    ),
                  },
        ...
        <section style={{ background: '#fff', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
          <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
              Perguntas sobre jaleco<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>branco</em>
            </h2>
            <FaqAccordion />
          </div>
        </section>
          ...
          <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: '2px', background: '#e5e0d8', marginTop: '3rem' }}>
              {posts.length > 0 ? posts.map(post => {
                ...
                [
                  { title: 'Como lavar e conservar seu jaleco branco profissional', href: '/blog/como-lavar-jaleco', tag: 'Cuidados', excerpt: 'Erros simples de lavagem aceleram o amarelamento e encurtam a vida do jaleco. Veja o guia completo.' },
                  { title: 'Jaleco branco: tradição e protocolos na saúde', href: '/blog', tag: 'Guia', excerpt: 'Por que o branco domina a área da saúde e o que profissionais recomendam sobre cor e vestimenta.' },
        ...
        <section style={{ background: '#f9f7f4', padding: 'clamp(5rem,10vw,9rem) clamp(1.5rem,5vw,4rem)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2.5rem,5vw,4.5rem)', fontWeight: 400, lineHeight: 1.1, maxWidth: 700, margin: '0 auto 1rem', color: '#1a1a1a' }}>
              O jaleco branco certo<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>faz a diferença</em>
            </h2>
