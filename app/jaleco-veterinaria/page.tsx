...
export const metadata: Metadata = {
  title: { absolute: 'Jaleco de Veterinária Slim — Feminino e Masculino na Jaleca' },
  description: 'Nota 4,9 no Google. Jaleco com elastano para veterinárias — branco, preto e colorido, PP ao G3. Frete grátis Sudeste acima de R$499. Troca em 7 dias.',
  alternates: { canonical: 'https://jaleca.com.br/jaleco-veterinaria' },
  openGraph: {
    title: 'Jaleco para Veterinária — Slim, Branco e Colorido — Jaleca',
    description: 'Nota 4,9 no Google. Jaleco para veterinárias com elastano, modelagem slim, PP ao G3. Frete grátis Sudeste acima de R$499.',
    url: 'https://jaleca.com.br/jaleco-veterinaria',
    siteName: 'Jaleca',
    locale: 'pt_BR',
    type: 'article',
  },
  twitter: {
    ...
    title: 'Jaleco para Veterinária — Tecido Premium, Caimento Perfeito — Jaleca',
    description: 'Jaleco premium para veterinárias. Elastano, caimento perfeito, do PP ao G3. Frete grátis Sudeste.',
...
const schemaArticle = {
  '@context': 'https://schema.org',
  '@type': 'Article',
  headline: 'Jaleco para Veterinária: Tecido Premium, Caimento Perfeito',
...
const breadcrumbSchema = {
  ...
  itemListElement: [
    ...
    { '@type': 'ListItem', position: 3, name: 'Jaleco para Veterinária', item: 'https://jaleca.com.br/jaleco-veterinaria' },
  ],
}
...
async function getJalecos(): Promise<WooProduct[]> {
  try {
    ...
    const slugs = PROFESSION_PRODUCT_SLUGS['veterinaria'] ?? []
...
export default async function JalecoVeterinarioPage() {
  ...
  return (
    <>
      ...
      {(() => { const arr = buildProductListSchema(produtos, 'https://jaleca.com.br/jaleco-veterinaria'); return arr ? arr.map((s, i) => <script key={'p'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
      {(() => { const arr = buildReviewSchema(placeData?.reviews, 'https://jaleca.com.br/jaleco-veterinaria', "Jaleco para veterinaria", produtos); return arr ? arr.map((s, i) => <script key={'r'+i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s).replace(/</g, '\\u003c') }} />) : null })()}
        ...
        <div style={{ background: '#f9f7f4', borderBottom: '1px solid #e5e0d8', padding: '0.85rem clamp(1.5rem,5vw,4rem)' }}>
          <ol className="flex items-center gap-2 max-w-[1200px] mx-auto" style={{ listStyle: 'none' }}>
            {[
              ...
              { label: 'Para Veterinária', href: null },
            ].map((crumb, i, arr) => (
          ...
          <section id="produtos" className="px-4 py-12 lg:px-16 lg:py-20" style={{ background: '#f9f7f4', padding: 'clamp(4rem,8vw,7rem) clamp(1.5rem,5vw,4rem)' }}>
            <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto' }}>
              <div className="mb-10">
                <div>
                  ...
                  <h2 style={{ fontFamily: "'Cormorant', Georgia, serif", fontSize: 'clamp(2rem,3.5vw,3rem)', fontWeight: 400, lineHeight: 1.15, color: '#1a1a1a' }}>
                    Jalecos para<br /><em style={{ fontStyle: 'italic', fontWeight: 300 }}>Veterinárias</em>
                  </h2>
                </div>
                </div>
              ...
              <div className="flex justify-center mt-8">
                <Link href={getVerMaisUrl('veterinaria')} style={{ display: 'inline-flex', padding: '0.9rem 2rem', fontSize: '0.78rem', fontWeight: 400, letterSpacing: '0.14em', textTransform: 'uppercase', textDecoration: 'none', border: '1px solid #1a1a1a', color: '#1a1a1a' }}>
                  Ver mais →
                </Link>
              </div>
            </div>
