...
- **Google Ads com 0 conversões históricas** (últimos 7 dias e acumulado ~3 meses: R$1.294,25). Em 29/04 o `GA4_MEASUREMENT_PROTOCOL_SECRET` foi corrigido no Vercel; aguardar 7-14 dias de dados reais para validar importação GA4→Ads.
...
- **SEO**: Corrigir `lastmod` idêntico no sitemap; criar rotas gender-split `/categoria/jaleco-feminino` etc.
...
- **2026-04-28**: Claude Code ajustou estrutura JSX dos 3 arquivos localmente; `npx tsc --noEmit` passou; não commitado nem deployado (superado pelo deploy de 29/04).
...
- **ALERTA ADS (29/04)**: Google Ads acumulou R$1.294,25 em ~3 meses com 0 conversões históricas — bug de rastreamento parcialmente corrigido (GA4 MP secret adicionado). Meta Ads: R$1.745,60 gastos, 21 compras registradas (~R$83/compra). Total mídia paga ~R$3.040. Aguardar 7-14 dias de dados reais antes de rebalancear budget.
